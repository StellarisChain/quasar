...
export function pointToBytes(point: any, addressFormat: AddressFormat = AddressFormat.FULL_HEX): Uint8Array {
    if (addressFormat === AddressFormat.FULL_HEX) {
        ...
        // Stellaris custom compressed format: prefix 42/43 + x coordinate (33 bytes total)
        // This matches the Python stellaris-wallet implementation exactly:
        // - Prefix 42 for even y-coordinates -> addresses start with "D"
        // - Prefix 43 for odd y-coordinates -> addresses start with "E"
        // - X coordinate in little-endian format (32 bytes)
        const x = point.x;
        const y = point.y;
        // Use prefix 42 for even y, 43 for odd y (matching Python stellaris-wallet)
        const prefix = (y % BigInt(2) === BigInt(0)) ? 42 : 43;
        // For the x coordinate, use little-endian as per Python implementation
...
// Helper function for little-endian byte conversion (matching Python implementation)
// This ensures addresses generated by quasar match those from stellaris-wallet Python
function intToBytesLE(num: number | bigint, length: number): Uint8Array {
    let n = typeof num === 'bigint' ? num : BigInt(num);
    const arr = new Uint8Array(length);
    for (let i = 0; i < length; i++) {
        arr[i] = Number((n >> BigInt(8 * i)) & BigInt(255));
    }
    return arr;
}
